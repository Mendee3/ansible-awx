...
#
# NOTE (AWX rollout): on hosts without outbound internet access, run with
#   --skip-auditd-rules --skip-sysmon --splunk-user splunk
# so only the offline-safe steps run (auditd install/tuning, rsyslog/journald
# verification, Splunk UF read-permission grants). The full script downloads
# the Neo23x0 rules from raw.githubusercontent.com and Sysmon for Linux from
# packages.microsoft.com, which requires outbound internet on the target host.
# =============================================================================
